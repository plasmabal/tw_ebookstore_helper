...
**適用平台**：Readmoo 書籍詳情頁（`/book/`）

### 2.1 顯示條件

- 頁面需有 `電子書售價` 或 `電子書特價` 文字的 `.price` 區塊
- 注入位置：該價格區塊內（`info.container`）
- 若已注入過（`.teh-price-helper-container` 已存在），不重複注入
...
- 「取消」：關閉浮窗，不儲存
- 「儲存」：寫入 `chrome.storage.local`（`wishlistRemarks`、`wishlistTags`），關閉浮窗
...
- **即時同步**：`chrome.storage.onChanged` 監聽，儲存變更後自動重新渲染頁面上的所有功能
