...
*   **瀏覽器工具優先權 (Browser Tool Priority)**：當任務需要存取網頁結構與內容時，必須優先使用 `agent-browser` 技能，嚴禁在未經確認的情況下使用內建的 `browser_subagent`。若遇到特定工作必須使用內建工具才能完成時，必須先獲得使用者的明確同意。
...
## 7. 更新日誌 (CHANGES.md) 管理規範
*   **Upcoming Version**：所有新功能或修正應先紀錄於最上方的 `## [Upcoming Version]` 區塊。
